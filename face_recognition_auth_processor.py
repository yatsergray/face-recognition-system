...
def run():
    ...
    file_names = ["recognized_face", "left_frame_face", "not_recognized_face", "not_single_face"]
    file_name_index = 3

    output_file = f"{file_names[file_name_index]}.mp4"
    frame_width = int(video_capture.get(3))
    frame_height = int(video_capture.get(4))
    fps = 10

    gif_codec = cv2.VideoWriter_fourcc(*"mp4v")
    gif_writer = cv2.VideoWriter(output_file, gif_codec, fps, (frame_width, frame_height))
